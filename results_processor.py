...
# -------------------------
# Helper functions to fix import errors
# -------------------------
def binary_probability(probability: float, threshold: float = 0.5) -> int:
    """
    Convert a probability into a binary prediction.
...
def confidence_score(probability: float) -> float:
    """
    Simple helper to return a confidence score.
    Currently just returns the input probability.
    """
    return float(probability)

# -------------------------
# Main ResultsProcessor class
# -------------------------
class ResultsProcessor:
    ...
    def compute_local_ensemble_results(self,
        ...
        media_name = first_result.get(
            "media_name",
            os.path.basename(media_path) if media_path != "unknown_path" else "unknown_name"
        )
        ...
        if method == "voting":
            ...
            probabilities = [r["probability"] for r in valid_outputs]
            if probabilities:
            ...
            console.print(f"[yellow]Unsupported local ensemble method '{method}'. Defaulting to undetermined.[/yellow]")
        ...
        for model_name, model_specific_results in results_grouped.items():
            ...
            for result in model_specific_results:
                if result.get("ground_truth") == "Unknown" or \
                   ...
                   result.get("probability") is None: 
                    continue
            ...
            try:
                ...
                if not (len(np.unique(y_pred_proba)) < 2 and len(y_pred_proba) == len(y_true)):
                    try: 
                        current_metrics["auc"] = auc(*roc_curve(y_true, y_pred_proba)[:2])
                    except ValueError: 
                        current_metrics["auc"] = np.nan
